...
void selectSync(const FunctionCallbackInfo<Value>& args) {
	...
	char query[1000];
	unsigned int i,k,limit,chunk_length,flag=0;
	int j,temp;
	SQLRETURN retcode;	
	SQLSMALLINT colCount = 0;
	SQLLEN indicator;
	...
	//extracting chunk_length
	temp = (int)args[2]->NumberValue();
	if(temp <= 0){
		chunk_length = 4294967294;
	}
	else{
		chunk_length = temp;
	}
	...
	Local<Array> end_result = Array::New(isolate);
	Local<Object> obj;
	...
	for(i=0;i<limit; i++) {
		...
		if(i%chunk_length == chunk_length-1){
			flag=0;
			result->Set(chunk_length-1,obj);
			Local<Value> argv[2] = { Number::New(isolate, retcode), result };
			cb->Call(isolate->GetCurrentContext()->Global(), 2, argv);
		}
		else{
			flag=1;
			result->Set(i%chunk_length,obj);
		}
	}
	if(flag!=0 || i==0){
